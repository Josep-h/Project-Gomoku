...
class GUI:
    ...
    def add_score(self, winner):
        ...
        self.show_messages("Game end")

    def render_step(self, action, player,num):
        """
        ...
        if self.last_action_player:     # draw a cross on the last piece to mark the last move
            self._draw_pieces(self.last_action_player[0], self.last_action_player[1], False,num)

        self._draw_pieces(action, player, True,num)
    ...
    def _draw_pieces(self, loc, player, last_step=False,num=0):
        """
        draw pieces
        ...
        self._draw_text(num , (int(self.UnitSize * 1.5 + x * self.UnitSize),
                                  int(self.UnitSize * 1.5 + (self.BoardSize - y - 1) * self.UnitSize)),
                        text_height=self.TestSize, font_color=c1)
        if last_step:
            if player == 1:
                c = (255, 255, 255)
                c1 = (0, 0, 0)
                ...
                c = (0, 0, 0)
                c1 = (255, 255, 255)
            self._draw_text(num-1, (int(self.UnitSize * 1.5 + x * self.UnitSize),
                                  int(self.UnitSize * 1.5 + (self.BoardSize - y - 1) * self.UnitSize)),
                            text_height=self.TestSize,font_color=c1)
